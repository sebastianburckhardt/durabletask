...
namespace DurableTask.EventHubs
{
    ...
    /// </summary>
    public class EventHubsOrchestrationService : IOrchestrationService, IOrchestrationServiceClient, IDisposable
    {
        // back-end
        ...
        internal EventHubsOrchestrationServiceSettings Settings { get; private set; }
        ...
        internal BatchWorker<PartitionEvent> BatchSender { get; private set; }
        ...
        private BatchTimer<CancellableCompletionSource<OrchestrationState>> ResponseTimeouts { get; set; }
        ...
        private void SendBatch(List<PartitionEvent> batch)
        {
            this.Queue.SendBatchAsync(outlist).Wait();
        }
        ...
        public async Task StartAsync()
        {
            ...
            this.ResponseTimeouts = new BatchTimer<CancellableCompletionSource<OrchestrationState>>(token, SendNullResponses);
            this.BatchSender = new BatchWorker<PartitionEvent>(token, this.SendBatch);
        ...
        private static void SendNullResponses<T>(IEnumerable<CancellableCompletionSource<T>> promises) where T : class
        {

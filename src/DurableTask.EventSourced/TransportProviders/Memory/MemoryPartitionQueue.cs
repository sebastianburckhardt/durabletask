...
namespace DurableTask.EventSourced.Emulated
{
    ...
    internal class MemoryPartitionQueue : MemoryQueue<PartitionEvent, byte[]>, IMemoryQueue<PartitionEvent>
    {
        ...
        protected override void Deliver(PartitionEvent evt)
        {
            try
            {
                evt.ReceivedTimestamp = partition.Stopwatch.Elapsed.TotalMilliseconds;
            ...
            {
                partition.ErrorHandler.HandleError(nameof(MemoryPartitionQueue), $"Encountered exception while trying to deliver event {evt} id={evt.EventIdString}", e, true, false);
            }
        }
    }
}
